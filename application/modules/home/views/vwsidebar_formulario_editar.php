			...
			$(document).ready(function(){
				...
				$("#main_content .formulario").submit(function(evt){
					...
					$.each($(".campo_editable"),function(i,item){
						...
						temp_array[x++]=$(item).attr("data-idcampos")!==undefined ? $(item).attr("data-idcampos") : 0;
						temp_array[x++]=$(item).attr("data-exp")!==undefined ? $(item).attr("data-exp") : 0;
						lis[i]=temp_array;
					});
					var id_exp=$("#main_content").attr("data-expediente");
					show_wait("box-warning","Guardando","Por favor espere.",1);
					$.post(window.url.base_url+"home/ctrhome/editar_formulario",{id_exp:id_exp, data:lis},function(resp){
						resp=JSON.parse(resp);
						if(resp.success!==false){
