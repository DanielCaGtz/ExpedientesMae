		...
		<script>
			$(document).ready(function(){
				...
				function get_li(nombre="Nuevo bloque de datos", id=0){
					var add="<li class='ui-state-default ui-sortable-handle'>";
						add+="<div class='box box-default collapsed-box'>";
							add+="<div class='box-header with-border'>";
								add+="<input data-id='"+id+"' type='text' style='border:none;width: 70%;font-size: 14px;' value='"+nombre+"'>";
				...
				$("#sortable").sortable();
				var prev_val_expedientes;
				$("#expedientes").focus(function(){
					prev_val_expedientes = $("#expedientes").val();
				}).change(function(){
					$("#loader").show();
					...
					if(s){
						$("#sortable").empty();
						$.post(window.url.base_url+"home/ctrhome/consultar_bloques_por_expediente",{id:$("#expedientes").val()},function(resp){
							resp=JSON.parse(resp);
							$("#save").attr("data-unsaved",0);
							$("#loader").hide();
							if(resp.success!==false){
								$.each(resp.result,function(i,item){
									$("#sortable").append(get_li(item.nombre, item.id));
								});
								$("#save").prop('disabled', false);
							}else $("#save").prop('disabled', false);
						});
					}else{
						$("#expedientes").val(prev_val_expedientes);
						$("#loader").hide();
						return false;
					}
				});
				$("#add").on("click",function(){
					if($("#expedientes").val()!="0"){
				...
				$("#save").on("click",function(){
					if($("#expedientes").val()!="0" && !$("#save").prop('disabled')){
						var lis=new Array();
						...
						$.post(window.url.base_url+"home/ctrhome/save_bloques_por_expediente",{data:lis, id:$("#expedientes").val()},function(resp){
							resp=JSON.parse(resp);
							if(resp.success!==false){
				...
				$("#sortable").on("click",".del",function(){
					...
					if(parseInt(id_main)>0){
						$.post(window.url.base_url+"home/ctrhome/consult_bloque_to_delete",{id:parseInt(id_main)},function(resp){
							resp=JSON.parse(resp);
							var s=false;
							if(resp.result===false){
								if(confirm("¿Realmente desea eliminar este elemento? Todos los campos relacionados se eliminarán.")) s=true;
							}else var s=true;
							if(s!==false){
								show_wait(1000,"box-warning","Guardando","Por favor espere.");
								$.post(window.url.base_url+"home/ctrhome/delete_bloque",{id:parseInt(id_main)},function(result){
