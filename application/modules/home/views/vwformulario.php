				...
				<div class="col-lg-6 col-xs-6">
					<div class="box <?php echo $expediente['color']; ?> box-solid">
						...
						<form role="form" class="form-contact formulario" style="background: #eaeaea;color: black;">
							<div class="box-body">
								<?php $data_campos=$controller->get_data("*","campos_expedientes","campos_bloques_id='".$bloque["id"]."'","orden"); if($data_campos!==FALSE) foreach($data_campos AS $e=>$key){ ?>
								<?php if(intval($key["is_secretcode"])==0){ ?>
								<div class="form-group">
							  		<label for="field_<?php echo $key['id']; ?>"><?php echo $key["nombre"]; ?></label>
							  		<?php echo $controller->get_field($key["id"]); ?>
								</div>
								<?php }else{ echo $controller->get_field($key["id"],0,$key["is_secretcode"]); } ?>
